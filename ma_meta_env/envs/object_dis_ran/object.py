...
The object's shape can be arbitrary, only need to know its centroid and mass
...
class HeavyObjectEnv(gym.Env):
    ...
    def _clip_actions(self, actions):
        if self.centralized:
            a=[]
            a[2]=actions%3-1
            a[1]=((actions-a[2])/3)%3-1
            a[0]=((actions-a[2]-3*a[1])/9)%3-1
            ...
            for a in actions:
                if a==0:
                    ...
                    b=[0.5,self.regulate_radians(a*np.pi/4)]
                action.append(np.array(b,dtype='float32'))
